...
export const registroSchema = yup.object().shape({
    ...
    usuario: yup.string().min(6, "Minimo 6 caracteres").required("Este campo es requerido"),
    clave: yup.string().required("Este campo es requerido").min(8, "minimo 8 caracteres").max(20, "Máximo 20 caracteres")
    ...
    confirmarClave: yup.string().oneOf([yup.ref("clave")], "la contraseña debe coincidir"),
    rolId: yup.string().required('Este campo es requerido').oneOf(['1','2'], "Seleccione una opción")
})
